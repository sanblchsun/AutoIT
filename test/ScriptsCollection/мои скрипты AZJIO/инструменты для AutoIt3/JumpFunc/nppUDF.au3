...
Global Const $IDM_EDIT_SELECTALL=($IDM_EDIT + 7) ; Выделяет весь текст
...
Global Const $IDM_EDIT_CURRENTDIRTOCLIP=($IDM_EDIT+31) ; Отправить текущее слово в буфер обмена
...
; Константы команд меню "Кодировки"
...
; Константы команд меню "Синтаксис"
...
Global Const $NPPM_GETCURRENTDOCINDEX=($NPPMSG + 23) ; Получает номер вкладки отсчёт от 0
...
; Высылает команды встроенному объекту SciTE
...
; #FUNCTION# ;=================================================================================
; Имя функции ...: _npp_GetText
; Описание ........: Получить текст
; Синтаксис.......: _npp_GetText ( [$flag = 2] )
; Параметры:
;		$flag - Флаг определяющий поведение
;                  |0 - Текущее слово
;                  |1 - Текущую строку
;                  |2 - Весь текст
; Возвращаемое значение: Успешно - Строка
;					Неудачно - 0
; ============================================================================================
Func _npp_GetText($flag = 2);$flag=0 - current word, $flag=1 - current line, $flag=2 - all text
	...
	$HndCtrl_Cur = ControlGetHandle('[CLASS:Notepad++]', "", "[CLASSNN:Scintilla" & _npp_GetCtrlInstance() & "]") ; Получает дескриптор Scintilla
...
; #FUNCTION# ;=================================================================================
; Имя функции ...: _npp_SetText
; Описание ........: Устанавливает текст
; Синтаксис.......: _npp_SetText ( $s_Text [, $flag = 2] )
; Параметры:
;		$s_Text - Текст
;		$flag - Флаг определяющий поведение
;                  |0 - Вставляет в конец документа
;                  |1 - Вставляет в позицию курсора
;                  |2 - Перезаписывает весь текст
...
; Возвращает текущий полный путь используя буфер обмена
...
; Возвращает номер экземпляра Scintilla
...
Func _npp_GetCurPos();0-based
	...
	If $s_Text Then
		...
		$ar[0] = _SendMessage(WinGetHandle('[CLASS:Notepad++]'), $NPPM_GETCURRENTLINE, 0, 0); 0-based (отсчёт от 0)
		$wparam = DllStructCreate("uint Start")
		$lparam = DllStructCreate("uint End")
		_SendMessage($HndCtrl_Cur, 0xB0, DllStructGetPtr($wparam), DllStructGetPtr($lparam), 0, "ptr", "ptr");$EM_GETSEL=0xB0
		$aSel[0] = DllStructGetData($wparam, "Start")
...
; #FUNCTION# ;=================================================================================
; Имя функции ...: _npp_SetCurPos
; Описание ........: Устанавливает курсор в указанную строку и столбец
; Синтаксис.......: _npp_SetCurPos ( [$iLine = -1 [, $iPos = 0]] )
; Параметры:
;		$iLine - Номер строки, отсчёт от 0
;		$iPos - Номер столбца (количество символов от начала строки), отсчёт от 0
...
Func _npp_SetCurPos($iLine = -1, $iPos = 0) ; 0-based
	...
	$HndCtrl_Cur = ControlGetHandle('[CLASS:Notepad++]', "", "[CLASSNN:Scintilla" & _npp_GetCtrlInstance() & "]") ; Получает дескриптор Scintilla
	If Not $HndCtrl_Cur Then Return 0 ; если не дескриптор, то
	If $iLine < 0 Or $iPos < 0 Then ; Если отрицательные значения, то      получаем длинну текста чтобы установить курсор в конец документа
		$iLength = _SendMessage($HndCtrl_Cur, 0x000E);$__EDITCONSTANT_WM_GETTEXTLENGTH=0x000E
		_SendMessage($HndCtrl_Cur, 0xB1, $iLength, $iLength);$EM_SETSEL=0xB1
			MsgBox(0, 'Сообщение', $iLength)
	Else
		$str = _npp_GetText() ; Получает весь текст
		If $str Then
