...
#AutoIt3Wrapper_OutFile=JumpToString.exe
#AutoIt3Wrapper_icon=JumpToString.ico
#AutoIt3Wrapper_Compression=n
#AutoIt3Wrapper_UseUpx=n
#AutoIt3Wrapper_UseAnsi=y
#AutoIt3Wrapper_Res_Comment=-
#AutoIt3Wrapper_Res_Description=JumpToString.exe
...
Global $Gui1, $TrSort=1, $OnTop=0, $Ini=@ScriptDir&'\JumpToString.ini'
...
Func _AddItem(ByRef $aFunc, $Parent, $x)
	...
	For $i = 0 to $tmp-1 Step 2
		...
		$aTmp[$i/2][0]=StringTrimRight($aFunc[$i+1], 1)
...
Func _AddItem2($AllText)
	...
	$TmpAll=StringRegExpReplace($TmpAll, '(?ms)^[^;]*?\r\n', '') ; удаляет строки без комментариев
	$aComments=StringRegExp($TmpAll, $RedExp3, 3) ; добавляем комментарии скрипта
	If Not @error Then
		$tmp=UBound($aComments)
		Local $aTmp[$tmp/2][4]
		For $i = 0 to $tmp-1 Step 2
			If $aComments[$i]='#cs' And $aComments[$i+1]='' Then $aComments[$i+1]='#cs'
			If StringLeft($aComments[$i+1], 3)='==>' Then ContinueLoop
