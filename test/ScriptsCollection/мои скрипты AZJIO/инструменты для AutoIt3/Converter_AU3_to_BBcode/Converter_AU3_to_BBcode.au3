...
#Obfuscator_Ignore_Variables=$TrNoCode, $TrB_KW, $TrB_Var, $Tri_Com, $TrCB, $TrBB, $TrEn, $TrCheck, $tmpNoCode, $tmpB_KW, $tmpB_Var, $tmpi_Com, $tmpCB, $tmpBB, $tmpEn, $tmpCheck
...
Func _ConvCMD($input, $byf=0)
	...
	If $tmpBB=1 Then
		...
		Next
		$HTML = StringReplace($HTML, '&nbsp;', ' ') ; замена HTML-пробелов
		...
		Do
			$HTML=StringRegExpReplace($HTML, '(\[color=#('&$H16&')\][^\[]*?)\[/color\]([\s]*)\[color=#\2\]', '\1\3')
		Until @Extended=0
		$HTML=StringReplace($HTML, ' 3ksl4¤nl3r01k ', '[') ; восстановление символа временно заменённого
		
		; $HTML=StringRegExpReplace($HTML, '(\[color=#('&$H16&')\][^\[]*?)\[/color\]([	 \r\n]*)\[color=#\2\]', '\1\3')
		; $HTML=StringRegExpReplace($HTML, '(?m)\[color=#'&$H16&'\][ 	]*?\[/color\]', '')

		; формат шрифта
		If $tmpB_KW=1 Then $HTML=StringRegExpReplace($HTML, '(?si)(\[color=#'&$aThColor[5]&'\].*?\[/color\])', '[b]$1[/b]')
		If $tmpB_Var=1 Then $HTML=StringRegExpReplace($HTML, '(?si)(\[color=#'&$aThColor[9]&'\].*?\[/color\])', '[b]$1[/b]')
		If $tmpi_Com=1 Then
			$HTML=StringRegExpReplace($HTML, '(?si)(\[color=#'&$aThColor[1]&'\].*?\[/color\])', '[i]$1[/i]')
			; $HTML=StringRegExpReplace($HTML, '(?si)(\[color=#'&$aThColor[2]&'\].*?\[/color\])', '[i]$1[/i]')
		EndIf
	...
	Else
		...
		$HTML = StringRegExpReplace($HTML, '(<span class="S(\d)">)((?:&nbsp;|[ ])+)(</span>)', '\3') ; удаляем теги пробелов, которые не могут быть цветными

		; $HTML=StringTrimLeft($HTML, 2)
		
		; удаление последовательных одинаковых тегов
		Do
			$HTML=StringRegExpReplace($HTML, '(<span class="S(\d)">[^<]*?)</span>([\s]*)<span class="S\2">', '\1\3')
		Until @Extended=0
...
$font="Arial"
	...
	GUICtrlCreateLabel($LngVer&' 0.2  01.11.2011', 55, 100, 210, 17)
