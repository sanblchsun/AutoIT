...
Func SciTE_Start() ; запускает SciTE
...
; 3.3.8.1 и выше (с перекодировкой имён в Юникод)
Func SciTe_Convert($aData, $sFileInput, $sFileOutput) ; $SciTE_hwnd
    Local $SciTE_hwnd = $aData[0]
    SendSciTE_Command(0, $SciTE_hwnd, 'open:' & BinaryToString(StringToBinary(StringReplace($sFileInput, "\", "\\"), 4)) & '')
    SendSciTE_Command(0, $SciTE_hwnd, 'exportashtml:' & BinaryToString(StringToBinary(StringReplace($sFileOutput, "\", "\\"), 4)))
    SendSciTE_Command(0, $SciTE_hwnd, 'close:')
EndFunc
