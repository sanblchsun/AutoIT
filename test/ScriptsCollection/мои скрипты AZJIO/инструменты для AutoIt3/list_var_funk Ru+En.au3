...
Global $k, $kf, $text01, $text02, $text0, $text, $Info_Edit1, $Info_Edit2, $Time, $timer, $FileName=''
...
While 1
	...
	Select
		...
		Case $msg = -13
			If StringRight(@GUI_DragFile, 4)='.au3' And Not StringInStr(FileGetAttrib(@GUI_DragFile), "D") Then
				...
				GUICtrlSetData($per, $LngLVr&' - '&$k&' '&$LngLPc)
				GUICtrlSetData($fun, $LngLFu&' - '&$kf&' '&$LngLPc)
				WinSetTitle($Gui, '', StringRegExpReplace(@GUI_DragFile, '(^.*)\\(.*)$', '\2')&' - '&$LngTitle)
			EndIf
			If StringInStr(FileGetAttrib(@GUI_DragFile), "D") Then
				_Include(@GUI_DragFile)
			EndIf
		Case $msg = -3
			Exit
	EndSelect
