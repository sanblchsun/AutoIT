...
Func _Delete()
	...
	$ErrDel = ''
	If $Tr7 Then _GUICtrlListView_BeginUpdate($Dubl)
	For $i = $aArr[0][0] To 1 Step -1
		If _GUICtrlListView_GetItemChecked($Dubl, $aArr[$i][3]) = 1 And FileExists($aArr[$i][0]) Then
			FileSetAttrib($aArr[$i][0], "-RST")
			If FileDelete($aArr[$i][0]) Then
				$kol += 1
				_GUICtrlListView_DeleteItem($Dubl, $aArr[$i][3])
				$aArr[$i][0] = ''
				$aArr[$i][3] = ''
			Else
...
Func _AllChec()
	If $aSizePath Then
		...
		If GUICtrlRead($hAllChec) = 1 Then
			$TempMD5 = ''
			For $i = 1 To $aArr[0][0]
				If $aArr[$i][2] = $TempMD5 Then _GUICtrlListView_SetItemChecked($Dubl, $aArr[$i][3])
				$TempMD5 = $aArr[$i][2]
			Next
		Else
			_GUICtrlListView_SetItemChecked($Dubl, -1, False)
		EndIf
		If $Tr7 Then _GUICtrlListView_EndUpdate($Dubl)
	EndIf
EndFunc   ;==>_AllChec
...
Func _SearchDubl()
	...
	For $i = 1 To $aArr[0][0] ; MD5
		...
		If $aArr[$i][2] <> $TempMD5 Then
			...
			_GUICtrlListView_InsertGroup($Dubl, -1, $GP, '---' & $GP & '---') ; добавляем группу
			; GUICtrlSetColor(-1,0x777777)
			; GUICtrlSetBkColor ( -1, 0xffffff )
			$aArr[$i][3] = _GUICtrlListView_AddItem($Dubl, $aArr[$i][0])
			; GUICtrlSetColor(-1,0x00B3A7)
			_GUICtrlListView_SetItemGroupID($Dubl, $aArr[$i][3], $GP)
		Else
			$ch += 1
			$aArr[$i][3] = _GUICtrlListView_AddItem($Dubl, $aArr[$i][0])
			_GUICtrlListView_SetItemGroupID($Dubl, $aArr[$i][3], $GP)
			_GUICtrlListView_SetItemChecked($Dubl, $aArr[$i][3])
		EndIf
		$TempMD5 = $aArr[$i][2]
	Next
...
Func _Dubl()
	$GuiPos = WinGetClientSize($Gui)
	$Dubl = _GUICtrlListView_Create($Gui, ' ', 10, 150, $GuiPos[0] - 20, $GuiPos[1] - 198, BitOR($LVS_NOCOLUMNHEADER, $LVS_REPORT, $LVS_SHOWSELALWAYS), $WS_EX_CLIENTEDGE, True)
	...
	; _GUICtrlListView_SetColumn($Dubl, 0, 'Дубликаты', 480)
EndFunc   ;==>_Dubl
		...
		Case $Dubl
			Switch $iCode
				Case $NM_DBLCLK
					...
					If $iItem > -1 Then
						$Tmp = _GUICtrlListView_GetItemText($Dubl, $iItem)
						If FileExists($Tmp) Then ShellExecute('"' & $Tmp & '"')
					EndIf
				Case $NM_RCLICK
					Local $aSel = _GUICtrlListView_GetSelectedIndices($Dubl, 1)
					Switch $aSel[0]
						Case 1
							$ItemText = _GUICtrlListView_GetItemText($Dubl, $aSel[1])
							$x = MouseGetPos(0)
							$y = MouseGetPos(1)
...
Func WM_SIZE($hwnd, $msg, $wParam, $lParam)
	...
	_WinAPI_MoveWindow($Dubl, 10, 150, $XYPos[0] - 20, $XYPos[1] - 198)
	Return $GUI_RUNDEFMSG
EndFunc   ;==>WM_SIZE
...
Func _ArrayUnique4($data)
	...
	$k = 1
	For $i = 1 To $data[0][0]
		Assign($data[$i][2] & '/', Eval($data[$i][2] & '/') + 1, 1)
		If Eval($data[$i][2] & '/') = 1 Then
			$data[$k][2] = $data[$i][2]
			$k += 1
		EndIf
	Next
