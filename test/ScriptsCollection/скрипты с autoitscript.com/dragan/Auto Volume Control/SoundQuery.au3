...
Func SoundSetGet($iMixerID, $sComponentType, $iComponentInstance, $sControlType, $fIsSet, $iNewParamValue)
	...
	Else
		...
		; Search through each source of each destination, looking for the indicated instance number for the indicated component type.
		Local $fFound = False
		Local $iCurDest = 0
	...
	If $fControlTypeIsBoolean Then
		...
		EndIf
	Else ; Assume the control can have more than just ON/OFF as its allowed states.
		; The MSDN docs imply that values fetched via the above method do not distinguish between left and right volume levels, unlike waveOutGetVolume().
