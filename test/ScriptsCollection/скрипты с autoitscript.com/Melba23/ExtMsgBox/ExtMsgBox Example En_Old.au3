...
While 1
	...
	Switch GUIGetMsg()
		...
		Case $hButton1
			...
			$iRetValue = _ExtMsgBox (@AutoItExe, "1|2|3|&4", "Test 1", $sMsg, 0, $hTest_GUI)
		...
		Case $hButton3
			...
			$sMsg &= "Note you can get && in button text" & @CRLF & @CRLF
			$sMsg &= "It will not time out"
		...
		Case $hButton6
			...
			$sMsg &= "It has no icon, no Taskbar button and TOPMOST is not set, with coloured text and background "
			$sMsg &= "and will time out in 20 seconds"

			$iRetValue = _ExtMsgBox (0, "Yes|No", "Test 6", $sMsg, 20, 100, 500)
