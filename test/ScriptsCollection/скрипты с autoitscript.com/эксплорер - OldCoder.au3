...
; OldCoder
; http://www.autoitscript.com/forum/topic/66495-windows-explorer-inside-my-form/page__view__findpost__p__493805
...
; TV functions
Global $TVM_INSERTITEM          = $TV_FIRST + 0
; Global $TVM_GETITEMRECT         = $TV_FIRST + 4
; Global $TVM_SETIMAGELIST            = $TV_FIRST + 9
Global $TVM_SETITEM             = $TV_FIRST + 13
Global Const $TVM_GETITEM = $TV_FIRST + 12
; Global $TVM_HITTEST             = $TV_FIRST + 17

; Global $TVI_FIRST                   = 0xFFFF0001
...
; Global $TVHT_ONITEM             = BitOr($TVHT_ONITEMICON, $TVHT_ONITEMLABEL, $TVHT_ONITEMSTATEICON)
; Global $TVHT_ABOVE              = 0x0100
; Global $TVHT_BELOW              = 0x0200
; Global $TVHT_TORIGHT                = 0x0400
; Global $TVHT_TOLEFT             = 0x0800

If Not IsDeclared("LVM_SETEXTENDEDLISTVIEWSTYLE")   Then Global $LVM_SETEXTENDEDLISTVIEWSTYLE   = 0x1036
If Not IsDeclared("LVM_SETCOLUMN")                  Then Global $LVM_SETCOLUMN              = 0x101A
If Not IsDeclared("LVCF_FMT")                       Then Global $LVCF_FMT                       = 0x0001
If Not IsDeclared("LVCFMT_RIGHT")                   Then Global $LVCFMT_RIGHT                   = 0x0001
If Not IsDeclared("LVM_GETHEADER")                  Then Global $LVM_GETHEADER              = 0x101F
...
While 1
    ...
    If IsArray($arInfo) AND $arInfo[4] = $nTreeView And $arInfo[2] And WinActive($hGui) Then
        $nFlag = 0
...
Func FillTreeRoot($arDrives)
    ...
    For $i = 1 To $arDrives[0]
		If $arDrives[$i]='a:' Then ContinueLoop
        $nChildren = 0
