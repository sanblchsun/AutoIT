...
Local $i, $nAngle, $level, $low, $high, $fps, $aWave
Local $color1[3]
Local $color2 = $color1
Local $dist1 = $H2 - 40
Local $dist2 = $H2 + 40
...
While Sleep(20)
	...
	If $current_pos >= $song_length Then _BASS_ChannelPlay($hStream, 1)

	$level = _BASS_ChannelGetLevel($hStream)
	$high = BitShift($level, 16)
	$low = BitAND($level, 0x0000FFFF)

	$aWave = _BASS_EXT_ChannelGetWaveformEx($hStream, 512, _
															0, $dist1, $W, $H6, _
															0, $dist2, $W, $H6)
