; http://www.autoitscript.com/forum/topic/24678-analog-clock/page__st__40#entry192164
; Analog Clock
; Created April 2006
; By greenmachine
...
Global $facecolor = "0xFFFFFF"
Global $r, $g, $b, $Red, $Green,$Blue
Global $rgb2bgr = "0x00" & StringRight($facecolor,2) & StringMid($facecolor,5,2) & StringMid($facecolor,3,2)
...
$MenuChangeColor = GUICtrlCreateMenuitem ("Change face color", $ContextMenu, 5)
GUICtrlSetOnEvent ($MenuChangeColor, "ChangeFaceColor")
...
Func AllThatGoodStuff($FirstRun = 0)
    ...
    If $FirstRun = 1 Then
        $BkgrndPen = DllCall ($gdi_dll, "hwnd", "CreatePen", "int", "0", "int", "0", "hwnd", $rgb2bgr); small background pen
        $BackgroundPens[0] = $BkgrndPen[0]
        $BkgrndPen = DllCall ($gdi_dll, "hwnd", "CreatePen", "int", "0", "int", "2", "hwnd", $rgb2bgr); medium background pen
        $BackgroundPens[1] = $BkgrndPen[0]
        $BkgrndPen = DllCall ($gdi_dll, "hwnd", "CreatePen", "int", "0", "int", "3", "hwnd", $rgb2bgr); large background pen
        $BackgroundPens[2] = $BkgrndPen[0]
        $BlackPen = DllCall ($gdi_dll, "hwnd", "CreatePen", "int", "0", "int", "0", "hwnd", "0x0000f0fF"); small red pen (second hand)
...
;~ Func ChangeFaceColor()
;~ Opt("GUIOnEventMode", 0)
;~;#include <GUIConstants.au3>
;~ $ColorPicker = GUICreate("Face Color Chooser", 579, 200, 192, 125)
;~ GUISetBkColor(0xF1EFE2)
;~ $color1 = GUICtrlCreateLabel("", 24, 16, 84, 84)
;~ GUICtrlSetBkColor(-1, 0xF1EFE2)
;~ $choose1 = 3
;~ GUICtrlSetBkColor(-1, 0xF1EFE2)
;~ $input1 = GUICtrlCreateInput("", 24, 105, 84, 21, -1)
;~ GUICtrlCreateLabel("Red", 116, 15, 24, 17)
;~ GUICtrlSetBkColor(-1, 0xF1EFE2)
;~ GUICtrlCreateLabel("Green", 116, 39, 33, 17)
;~ GUICtrlSetBkColor(-1, 0xF1EFE2)
;~ GUICtrlCreateLabel("Blue", 116, 63, 25, 17)
;~ GUICtrlSetBkColor(-1, 0xF1EFE2)
;~ $Red = GUICtrlCreateSlider(156, 15, 370, 21)
;~ GUICtrlSetLimit(-1,255,0)
;~ $r = GUICtrlCreateLabel("",530,15,20,20)
;~ $Green = GUICtrlCreateSlider(156, 39, 370, 21)
;~ GUICtrlSetLimit(-1,255,0)
;~ $g = GUICtrlCreateLabel("",530,39,20,20)
;~ $Blue = GUICtrlCreateSlider(156, 63, 370, 21)
;~ GUICtrlSetLimit(-1,255,0)
;~ $b = GUICtrlCreateLabel("",530,63,20,20)
;~ $ok = GUICtrlCreateButton("OK",24,136,84,42)
;~ GUISetState(@SW_SHOW)
;~ Red()
;~ Green()
;~ Blue()
...
;~   $facecolor = GUICtrlRead($input1)
;~   MsgBox(1,"$facecolor",$facecolor)
;~;WinClose("Face Color Chooser")
;~  Exit
;~ EndIf
;~ If $msg = $ok Then
;~   $facecolor = GUICtrlRead($input1)
;~;MsgBox(1,"$facecolor",$facecolor)
;~;Exit
;~  GUISetState(@SW_HIDE,$ColorPicker)
;~  GUISwitch($GUI_Clock)
;~  GUISetState(@SW_SHOW,$GUI_Clock)
;~  Opt("GUIOnEventMode", 1)
;~   GUISetBkColor ($facecolor)
;~   $rgb2bgr = "0x00" & StringRight($facecolor,2) & StringMid($facecolor,5,2) & StringMid($facecolor,3,2)
;~;MsgBox(1,"$rgb2bgr",$rgb2bgr)
;~;AllThatGoodStuff(1)
;~  CheckClockStuff(1)
;~   $BkgrndPen = DllCall ($gdi_dll, "hwnd", "CreatePen", "int", "0", "int", "0", "hwnd", $rgb2bgr); small background pen
;~   $BackgroundPens[0] = $BkgrndPen[0]
...
Func ChangeFaceColor()
$facecolor = _ChooseColor(2)
;GUISetBkColor($dColor)
;MsgBox(1,"$dColor",$facecolor) 
    GUISwitch($GUI_Clock)
    GUISetState(@SW_SHOW,$GUI_Clock)
    Opt("GUIOnEventMode", 1)
    GUISetBkColor ($facecolor)
