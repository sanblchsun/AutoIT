...
Global Enum $g_ListView = 1, $g_hListView, $g_hListHeader, $g_sCurrentDir, $g_sDirHistory, $g_iCol, $g_iColWidths, $g_iCompColWidths, _
		$g_iCompCol, $g_bHidden, $g_hStatusbar, $g_idButton, $g_idCombo, $g_hCombo, $g_sCombo, $LocMax
...
OnAutoItExitRegister('__closedll')

Global Const $FOLDER_ICON_INDEX = __GUIImageList_GetFileIconIndex(@SystemDir, 0, 1)
Global Enum $g_Ext, $g_icon, $g_type, $extmax
Global $g_aFileExtentions[1][$extmax] = [[0, 0, 0]]
#cs
	$g_aFileExtentions[$i][$type] = file type
	$g_aFileExtentions[$i][$icon] = Icon Index
#ce
...
Func _ExpFrame_Create($hFrameHandle, $sStartDir = 'My Computer', $bShowHidden = False)
	...
	$g_aExpFrames[$iIndex][$g_ListView] = GUICtrlCreateListView("Name", 0, $iBHeight + 1, $aClient[0], $aClient[1] - ($iSBHeight + $iSBVBorderWidth + 1) - $iBHeight, BitOR($LVS_SHAREIMAGELISTS, $WS_TABSTOP))
	...
	_GUICtrlListView_SetImageList($g_aExpFrames[$iIndex][$g_hListView], __GUIImageList_GetSystemImageList(), 1)
...
Func _ExpFrame_WMNotify_Handler($hWnd, $iMsg, $iwParam, $ilParam)
	...
	Switch $iCode
		...
		Case $NM_RCLICK
			...
			Local $iIndex = __GetIndex($hWndFrom)
			If @error Then Return $GUI_RUNDEFMSG
			If $hWndFrom = $g_aExpFrames[$iIndex][$g_hListHeader] Then __Columns_Header_RClick($hWndFrom)
	EndSwitch
...
Func __DirSetCurrent($iIndex, $sDirectory, $bPushHistory = True)
	...
	;If Not _GUICtrlListView_DeleteAllItems($g_aExpFrames[$iIndex][$g_hListView]) Then ConsoleWrite('error' & @CRLF)
	Local $aList = __FLTA($sDirectory)
	_GUICtrlListView_BeginUpdate($g_aExpFrames[$iIndex][$g_hListView])
	Local $Parent = _GUICtrlListView_AddItem($g_aExpFrames[$iIndex][$g_ListView], "[..]", $FOLDER_ICON_INDEX, 0)
	Local $UpdateTimer = TimerInit()
	Local $sFile, $sAttributes, $iIcon, $sItem, $iSize, $iTotalSize = 0, $iDirs = 0, $iFiles = 0
	Local $aExt, $sType, $iItem, $bytes = 0, $iListIndex = 1
	If IsArray($aList) Then
		For $i = 1 To $aList[0]
			If TimerDiff($UpdateTimer) > 250 Then
				Sleep(1);This is here to avoid Gui NOT Responding.
			...
			Else
				...
				$iTotalSize += $bytes
				$aExt = __Extention_GetInfo(StringRight($sFile, 4))
				$sType = $aExt[1]
				$iIcon = $aExt[2]
			EndIf
			$sItem = ''
			...
			_GUICtrlListView_SetItem($g_aExpFrames[$iIndex][$g_ListView], $aList[$i], $iListIndex, 0, $iIcon);set icon index
			$iListIndex += 1
		Next
	EndIf
...
; Remarks .......: None
;================================================================================================================================
Func __Columns_Header_RClick($hHeader)

	Local $iIndex = __GetIndex($hHeader)
...
Func __LVSort($hWnd, $nItem1, $nItem2, $nColumn)
	...
	If $aItem1Attb = $FOLDER_ICON_INDEX And $aItem2Attb <> $FOLDER_ICON_INDEX Then Return -1
	If $aItem2Attb = $FOLDER_ICON_INDEX And $aItem1Attb <> $FOLDER_ICON_INDEX Then Return 1
...
; #INTERNAL_USE_ONLY#============================================================================================================
; Name...........: __GUIImageList_GetSystemImageList
; Description ...: Returns system Imagelist
; Author ........: ProgAndy
; Modified.......: Beege
; Remarks .......: None
;================================================================================================================================
Func __GUIImageList_GetSystemImageList($bLargeIcons = False)
	Local $FILE_ATTRIBUTE_NORMAL = 0x00000080, $SHGFI_USEFILEATTRIBUTES = 0x10, $SHGFI_SYSICONINDEX = 0x4000, $SHGFI_SMALLICON = 0x1;, $SHGFI_LARGEICON = 0x0;,$FILE_ATTRIBUTE_NORMAL = 0x80
	...
	Local $return = DllCall('shell32.dll', "DWORD*", "SHGetFileInfo", "str", '.txt', "DWORD", $FILE_ATTRIBUTE_NORMAL, "ptr", DllStructGetPtr($FileInfo), "UINT", DllStructGetSize($FileInfo), "UINT", $dwFlags)
	If @error Then Return SetError(@error, 0, 0)
	Return $return[0]
EndFunc   ;==>__GUIImageList_GetSystemImageList
; #INTERNAL_USE_ONLY#============================================================================================================
; Name...........: __GUIImageList_GetFileIconIndex
; Description ...: Returns file icon index
; Author ........: ProgAndy
; Modified.......: Beege
; Remarks .......: None
;================================================================================================================================
Func __GUIImageList_GetFileIconIndex($sFileSpec, $bLargeIcons = False, $bForceLoadFromDisk = False);modified
	Local $FileInfo = DllStructCreate("dword hIcon; int iIcon; DWORD dwAttributes; CHAR szDisplayName[255]; CHAR szTypeName[80];")
	Local $dwFlags = BitOR(0x4000, 0x1)
	If Not $bForceLoadFromDisk Then $dwFlags = BitOR($dwFlags, 0x10)
	DllCall('shell32.dll', "DWORD*", "SHGetFileInfo", "str", $sFileSpec, "DWORD", 0x00000080, "ptr", DllStructGetPtr($FileInfo), "UINT", DllStructGetSize($FileInfo), "UINT", $dwFlags)
	If @error Then Return SetError(1, 0, -1)
	Return DllStructGetData($FileInfo, "iIcon")
EndFunc   ;==>__GUIImageList_GetFileIconIndex
#EndRegion Listview Icon Functions

#Region Extention functions
; #INTERNAL_USE_ONLY#============================================================================================================
; Name...........: __Extention_GetInfo
; Description ...: Returns an array containing file type and icon index
; Author ........: Beege
; Remarks .......: None
;================================================================================================================================
Func __Extention_GetInfo($sEXT)

	Static $sExtentions = ''

	Local $iElement, $iSearch = StringInStr($sExtentions, $sEXT)
	If $iSearch Then
		$iElement = ($iSearch + 3) / 4
	Else
		$sExtentions &= $sEXT
		$iElement = __Extention_AddtoArray($sEXT)
	EndIf

	Local $aExt[3] = [$g_aFileExtentions[$iElement][$g_Ext], $g_aFileExtentions[$iElement][$g_type], $g_aFileExtentions[$iElement][$g_icon]]
	Return $aExt

EndFunc   ;==>__Extention_GetInfo
; #INTERNAL_USE_ONLY#============================================================================================================
; Name...........: __Extention_AddtoArray
; Description ...: Used to add extension information to array
; Author ........: Beege
; Remarks .......: None
;================================================================================================================================
Func __Extention_AddtoArray($sEXT)

	Local $sType = DllStructCreate('wchar[1024]')
	ReDim $g_aFileExtentions[UBound($g_aFileExtentions) + 1][UBound($g_aFileExtentions, 2)]
	$g_aFileExtentions[0][0] += 1

	DllCall('shlwapi.dll', 'uint', 'AssocQueryStringW', 'dword', 0, 'dword', 3, 'wstr', $sEXT, 'wstr', '', 'ptr', DllStructGetPtr($sType), 'dword*', 1024)
	$g_aFileExtentions[$g_aFileExtentions[0][0]][$g_Ext] = $sEXT
	$g_aFileExtentions[$g_aFileExtentions[0][0]][$g_icon] = __GUIImageList_GetFileIconIndex('x' & $sEXT)
	$g_aFileExtentions[$g_aFileExtentions[0][0]][$g_type] = DllStructGetData($sType, 1)

	Return $g_aFileExtentions[0][0]

EndFunc   ;==>__Extention_AddtoArray
#EndRegion Extention functions
#Region Other...
; #INTERNAL_USE_ONLY#============================================================================================================
; Name...........: __FLTA
