...
Func __RESH_ReplaceRichEditTags(ByRef $sCode)
	...
	If Not @error Then
		...
		For $i = 0 To UBound($aRicheditTags) - 1
			$sCode = StringReplace($sCode, $aRicheditTags[$i], StringReplace($aRicheditTags[$i], '\', '#', 0, 2), 0, 2)
		Next
	EndIf

	;escape characters for rtf code
	$sCode = StringRegExpReplace($sCode, '([\\{}])', '\\\1') ; (\\|{|})
	$sCode = StringReplace($sCode, @CR, '\par' & @CRLF, 0, 2)
	$sCode = StringReplace($sCode, @TAB, '\tab ', 0, 2)
...
Func __RESH_ReplaceCommentBlocks(ByRef $sCode, $sUpdateFunction = 0)
	...
	If $g_RESH_VIEW_TIMES Then ConsoleWrite('__RESH_ReplaceCommentBlocks = ' & TimerDiff($time) & @LF)
EndFunc   ;==>__RESH_ReplaceCommentBlocks

Func __RESH_MarkQuotedStrings(ByRef $sCode, $sUpdateFunction = 0)
	Local $time = TimerInit()

	Local Const $iDouble = 34, $iSingle = 39

	Local $bState_Double = False, $bState_Single = False

	;find 3 character that are not being using in the script
	For $i = 1 To 255
		$g_iTagBegin = $i
		$g_iTagEnd = $i + 1
		$g_iTagComment = $i + 2
		If Not StringRegExp($sCode, '[' & Chr($i) & Chr($i + 1) & Chr($i + 2) & ']') Then ExitLoop
	Next

	;split code into ascii array
	Local $sNew, $aCode = StringToASCIIArray($sCode)
			...
			Case 34; (")
				If Not $bState_Single Then
					If $bState_Double Then
						$sNew &= $aCode[$i] & '|' & $g_iTagEnd & '|'
					Else
						$sNew &= $g_iTagBegin & '|' & $aCode[$i] & '|'
					EndIf
	...
	$aNew = StringSplit(StringTrimRight($sNew, 1), '|', 2)
	$sNew = StringFromASCIIArray($aNew)

	;remove tags for double string quotes
	$sCode = StringReplace($sNew, Chr($g_iTagEnd) & Chr($g_iTagBegin), '', 0, 2)
...
Func __RESH_Operators(ByRef $sCode)
	...
	$sCode = StringRegExpReplace($sCode, '([()[\]<>.*+=&^,/-])', '\\' & $g_cOperators & ' \1\\cf0 ')
...
Func __RESH_Special(ByRef $sCode, $sUpdateFunction = 0)
	...
	For $i = 0 To UBound($aSpec) - 1
		...
		$sCode = StringReplace($sCode, $aSpec[$i], '\' & $g_cSpecial & ' ' & $sRep & '\cf0', 0, 2)
	Next
...
Func __RESH_Functions(ByRef $sCode, $sUpdateFunction = 0)
	...
	For $i = 0 To 4
		__RESH_UpdateCallback($aUpdates[$i], 'Functions...', $sUpdateFunction)
		$sCode = StringRegExpReplace($sCode, "(?i)[^\$]\b(" & $aFunctions[$i] & ")\b", '\\' & $g_cFunctions & ' \0\\cf0 ')
	Next
...
Func __RESH_PreProcessor(ByRef $sCode, $sUpdateFunction = 0)
	...
	Local $sRep, $aIncludes = StringRegExp($sCode, '(?i)(?:\\cf12\h#include)(\V+\\par)', 3)
	$aIncludes = _ArrayRemoveDuplicates($aIncludes)
	For $i = 0 To UBound($aIncludes) - 1
		$sRep = StringRegExpReplace($aIncludes[$i], '\\cf\d\d?\h', '')
		If @extended Then
			$sRep = StringReplace($sRep, '<', '\' & $g_cString & ' <', 0, 2)
			If @extended Then $sCode = StringReplace($sCode, $aIncludes[$i], $sRep, 0, 2)
		EndIf
	Next
...
Func __RESH_Strings(ByRef $sCode, $sUpdateFunction = 0)
	...
	Local $aQuotes = StringRegExp($sCode, Chr($g_iTagBegin) & '\V*?' & Chr($g_iTagEnd), 3)
	...
	For $i = 0 To UBound($aQuotes) - 1
		...
		$sCode = StringReplace($sCode, $aQuotes[$i], $sRep, 0, 2)
	Next
...
Func __RESH_Comments(ByRef $sCode, $sUpdateFunction = 0)
	...
	$sCode = StringReplace($sCode, '\cf6 =\cf0 \cf6 =\cf0 \cf6 =\cf0 \cf6 =\cf0 ', '====', 0, 2)

	;remove color tags(even) from comments.
	Do
		$sCode = StringRegExpReplace($sCode, '(;\V*?)(?:\\cf\d\d?\h?)(\V*?)(?:\\cf\d\d?\h?)(\V*?\\par)', '\1\2\3')
	Until Not @extended
	;remove color tags(odd) from comments.
	$sCode = StringRegExpReplace($sCode, '(;\V*)(?:\\cf\d\d?\h?)(\V*\\par)', '\1\2')

	;remove any string markers that our in comments
	Do
		$sCode = StringRegExpReplace($sCode, '(;\V*)(?:[' & Chr($g_iTagBegin) & '|' & Chr($g_iTagEnd) & '])(\V*\\par)', '\1\2')
	Until Not @extended
	...
	$sCode = StringReplace($sCode, Chr($g_iTagComment), ';', 0, 2)

	If $g_RESH_VIEW_TIMES Then ConsoleWrite('Comments = ' & TimerDiff($time) & @LF)
...
Func __RESH_CleanUp(ByRef $sCode, $sUpdateFunction = 0)
	...
	For $i = 0 To UBound($aRegions) - 1
		If StringRegExp($aRegions[$i], '\\cf\d\d?\h') Then
			If Not StringInStr($aRegions[$i], '\' & $g_cComment) Then
				...
				If @extended Then $sCode = StringReplace($sCode, $aRegions[$i], $sRep, 0, 2)
			EndIf
		EndIf
	Next

	;removes duplicate cf1 tags..
	$sCode = StringReplace($sCode, '\cf1\cf1  ', '\cf1 ', 0, 2)
	...
	$sCode = StringReplace($sCode, '\cf0\cf11  ', '\cf11 ', 0, 2)
	...
	$sCode = StringReplace($sCode, Chr($g_iTagBegin), '\' & $g_cString & ' ', 0, 2)
	$sCode = StringReplace($sCode, Chr($g_iTagEnd), '\cf0 ', 0, 2)

	If $g_RESH_VIEW_TIMES Then ConsoleWrite('__RESH_CleanUp = ' & TimerDiff($time) & @LF)
...
Func __RESH_RestoreCommentBlocks(ByRef $sCode, $sUpdateFunction = 0)
	...
	For $i In $g_oUnique_Comments.keys()
		$sCode = StringReplace($sCode, $i, '\' & $g_cComment & ' ' & $g_oUnique_Comments.item($i) & '\cf0 ', 0, 2)
	...
	If $g_RESH_VIEW_TIMES Then ConsoleWrite('__RESH_RestoreCommentBlocks = ' & TimerDiff($time) & @LF)
