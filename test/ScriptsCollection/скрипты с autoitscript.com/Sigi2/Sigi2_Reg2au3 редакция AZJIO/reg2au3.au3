...
if 1 then
	;регистрация в реестре и копирование в системную папку при первом запуске. Если не желаете добавлять, но избавиться от вопросов, то удалите этот блок
	If Not FileExists(@SystemDir&'\reg2au3.au3') Then
		If MsgBox(4, "Выгодное предложение", 'Хотите скопировать скрипт в системную папку'&@CRLF&'и добавить в контекстное меню reg-файлов?')=6 Then
			FileCopy(@ScriptDir&'\reg2au3.au3', @SystemDir,1)
			RegWrite("HKCR\regfile\shell\mconv","","REG_SZ","Конвертировать reg2au3")
			RegWrite("HKCR\regfile\shell\mconv\command","","REG_SZ",@AutoItExe&' "'&@SystemDir&'\reg2au3.au3" "%1"')
		EndIf
	EndIf

	If $CmdLine[0]=0 Then
		$file_in = FileOpenDialog("Выбор файла *.reg, для конвертирования в au3.", @ScriptDir & "", "reg-файл (*.reg)", 1 + 4 )
		If @error Then Exit
	Else
		$file_in=$CmdLine[1]
	EndIf
