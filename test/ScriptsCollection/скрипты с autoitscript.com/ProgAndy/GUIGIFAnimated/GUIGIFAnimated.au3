...
;~      $lSize = "byte"
                ConsoleWrite('@@ (' & @ScriptLineNumber & ') ' & @MIN & ':' & @SEC & '):: byte' & @CR);### DEBUGLINE
            Case $PropertyTagTypeShort
                $lProp = DllStructCreate("short[" & Ceiling($lSize / 2) & "]", DllStructGetData($tPropItem, "value"))
;~      $lSize = "ushort"
                ConsoleWrite('@@ (' & @ScriptLineNumber & ') ' & @MIN & ':' & @SEC & '):: short' & @CR);### DEBUGLINE

            Case $PropertyTagTypeLong
                $lProp = DllStructCreate("long[" & Ceiling($lSize / 4) & "]", DllStructGetData($tPropItem, "value"))
;~      $lSize = "long"
                ConsoleWrite('@@ (' & @ScriptLineNumber & ') ' & @MIN & ':' & @SEC & '):: long' & @CR);### DEBUGLINE
        EndSwitch
...
; ' Ubergabewert : lInBitmap = GDI+ Bitmapobjekt
...
; ' Beschreibung : Uberpruft, ob der EXIF-Tag LoopCount
; '                vorhanden sind
; ' Ubergabewert : lInBitmap = GDI+ Bitmapobjekt
; ' Ruckgabewert : True = LoopCount vorhanden
; '                False = LoopCount nicht vorhanden
; '------------------------------------------------------
;===============================================================================
;
Func _GDIplus_CheckForProperty( _
        ByRef $hImage, _
        $PropertyID)
...
;~                         ' wenn ja, dann Ruckgabewert setzen und
;~                         ' Schleife verlassen
                        $bRet = True
                        ExitLoop
                    EndIf
...
;                  |If $iTimerID matches no existing timer, a new timer is created 
...
Func _Timer_SetTimerEx($hWnd, $iElapse = 250, $sTimerFunc = "", $iTimerID = -1)
    ...
    If $iTimerID > -1 Then
        ...
        Next
    EndIf
    If $iTimerID = -1 Or Not $Timer_Exists Then ; create a new timer
