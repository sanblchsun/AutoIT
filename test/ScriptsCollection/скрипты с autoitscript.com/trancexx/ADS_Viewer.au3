...
Global $hButtonBrowse = GUICtrlCreateButton("&Open File...", 20, 28, 70, 25)
...
While 1
	...
	If $sFile Then
		...
		_ADS_DisplayStreams($sFile, "", $hEditADS, $hGUI)
	...
	If $hMyTimer Then
		If TimerDiff($hMyTimer) > 1500 Or WinGetHandle("[ACTIVE]") <> $hGUI Then
...
Func _ADS_ShowStreams($iAlgo, $sFile, ByRef $hTreeView, $hLabel = 0) ; Enumerates ADS-s using wanted algorithm
	...
	If IsArray($aArray) Then
		GUICtrlSetImage($hTreeView, @SystemDir & "\shell32.dll", 133)
		For $sElem In $aArray
...
Func _ADS_List_NtQuery($sFile)
	Local $hFile = _ADS_CreateFile($sFile, 0, 0x00000001, 3, 0) ; GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, SECURITY_ANONYMOUS
	...
	While 1
		...
		If $iAllocSize > 262144 Then ExitLoop ; this size would just be super-mad
...
Func _ADS_List_FindStream($sFile)
	...
	; I'm not inerested in that, only alternate streams.
	; I need to loop through all other streams and collect fom there.
	Local $i = 1, $aOut[1]
