...
;		- _ProcessUDGetMemInfo() may report incorrect info (or nothing at all)
;
; Main Functions:
...
Global $_PUDhNTDLL=DllOpen("ntdll.dll")	; DLLClose() will be done automatically on exit. [this doesn't reload the DLL]
...
;
; Returns:
;	Success: Array returned from DLLCall(), and if a DLLStruct was passed, it was probably updated. @error MAY be set:
;		@error = 4 = Size mismatch (not sure if this would be returned with 'STATUS_SUCCESS' though)
...
Func __PUDQueryProcess($hProcess,$iProcInfoClass,$vProcInfoData,$iProcInfoSz,$sProcInfoType="ptr")
	Local $aRet=DllCall($_PUDhNTDLL,"long","NtQueryInformationProcess","handle",$hProcess,"int",$iProcInfoClass,$sProcInfoType,$vProcInfoData,"ulong",$iProcInfoSz,"ulong*",0)
...
; Now read through the strings one by one
	...
	For $i=0 To 7
		...
		If $iCurMinLen>0 And $iCurMaxLen>2 Then
			$stString=DllStructCreate("wchar["&$iCurMaxLen&']')
			; Read from string pointer to $stString structure (*2 for Unicode length)
			_ProcessMemoryRead($hProcess,$pString,DllStructGetPtr($stString),$iCurMaxLen*2)
			If @error Then
...
;	  to be absolutely safe it would be best combined with _ProcessMemoryVirtualQuery(), index 3
	...
	$stEnv=DllStructCreate("ubyte[4096]")		; 4096 may be too big/small depending on strings
