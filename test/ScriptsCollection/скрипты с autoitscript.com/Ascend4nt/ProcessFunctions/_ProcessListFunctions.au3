...
Func __PFCreateToolHelp32Snapshot($iProcessID,$iFlags)
	...
	For $i=1 To 10
		$aRet=DllCall($_PFhKern32DLL,"handle","CreateToolhelp32Snapshot","dword",$iFlags,"dword",$iProcessID)
		If @error Then Return SetError(2,@error,-1)
		; INVALID_HANDLE_VALUE (-1) ?
		If $aRet[0]=-1 Then
			; Heap or Module Snapshot?  MSDN recommends retrying the API call if LastError=ERROR_BAD_LENGTH (24)
			If BitAnd($iFlags,1)=1 Or BitAND($iFlags,0x8)=0x8 Or BitAND($iFlags,0x10)=0x10 And _WinAPI_GetLastError()=24 Then ContinueLoop
...
Func _ProcessListEx($vFilter=0,$iMatchMode=0)
	...
	Local $bMatchMade=True,$iTotal=0,$iArrSz=100,$aProcesses[$iArrSz+1][5],$bFilterOn=False
	...
	; Get first process
	$aRet=DllCall($_PFhKern32DLL,"bool","Process32FirstW","handle",$hTlHlp,"ptr",$pStPointer)
	...
	While 1
		...
		$bMatchMade=True
		; Next process
		$aRet=DllCall($_PFhKern32DLL,"bool","Process32NextW","handle",$hTlHlp,"ptr",$pStPointer)
	WEnd
	__PFCloseHandle($hTlHlp)
...
Func _ProcessGetChildren($vProcessID)
	...
	EndIf
;~ 	ConsoleWrite("Process ID on entrance: "&$vProcessID&@CRLF)
...
;~ 	ConsoleWrite("_ProcessListHeaps: Process ID on entry:"&$vProcessID&@CRLF)
	...
	; Get first heap
	$aRet=DllCall($_PFhKern32DLL,"bool","Heap32ListFirst","handle",$hTlHlp,"ptr",$pHLPointer)
	While 1
		If @error Then
			$iErr=@error
			__PFCloseHandle($hTlHlp)
...
;~ 		ConsoleWrite("~>Heap ID for Heap32ListFirst/Next:"&$iHeapID&@CRLF)
		...
		$iHeapTotalSz=0
		; 1st Heap block..
		$aRet=DllCall($_PFhKern32DLL,"bool","Heap32First","ptr",$pHEPointer,"dword",$vProcessID,"ulong_ptr",$iHeapID)
		While 1
			If @error Then
				$iErr=@error
				__PFCloseHandle($hTlHlp)
			...
			; Next block on heap
			$aRet=DllCall($_PFhKern32DLL,"bool","Heap32Next","ptr",$pHEPointer)
		WEnd
		If $bHeapWalk And Not $bCompleteList Then $aHeapList[$iTotal][2]=$iHeapTotalSz
		; Next heap
		$aRet=DllCall($_PFhKern32DLL,"bool","Heap32ListNext","handle",$hTlHlp,"ptr",$pHLPointer)
	WEnd
	__PFCloseHandle($hTlHlp)
...
;~ 	ConsoleWrite("Process ID on entry:"&$vFilterID&@CRLF)
	Local $hTlHlp,$aRet,$iCurPID,$iCurTID,$iTotal=0,$iArrSz=100,$aThreads[$iArrSz+1][3]
	...
	; Get first thread
	$aRet=DllCall($_PFhKern32DLL,"bool","Thread32First","handle",$hTlHlp,"ptr",$pTEPointer)
	...
	While 1
		...
		; Next thread
		$aRet=DllCall($_PFhKern32DLL,"bool","Thread32Next","handle",$hTlHlp,"ptr",$pTEPointer)
	WEnd
	__PFCloseHandle($hTlHlp)
...
;	1 = match substring
;	2 = match regular expression
;	+4 = stop at first match (can add or BitOr() this value)
...
;	False returns strictly 32-bit modules for 32-bit processes.
;
...
Func _ProcessListModules($vProcessID,$sTitleFilter=0,$iTitleMatchMode=0,$bList32bitMods=False)
	If Not IsNumber($vProcessID) Then
		...
		If $vProcessID=0 Then Return SetError(1,0,"")
	EndIf
;~ 	ConsoleWrite("Process ID on entry:"&$vProcessID&@CRLF)

	Local $hTlHlp,$aRet,$sTitle,$bMatchMade=True,$iTotal=0,$bMatch1,$iArrSz
	...
	; Get first module
	$aRet=DllCall($_PFhKern32DLL,"bool","Module32FirstW","handle",$hTlHlp,"ptr",$pMEPointer)
	...
	While 1
		...
		If IsString($sTitleFilter) Then
			Switch $iTitleMatchMode
				Case 0
					If $sTitleFilter<>$sTitle Then $bMatchMade=False
				Case 1
					If StringInStr($sTitle,$sTitleFilter)=0 Then $bMatchMade=False
				Case Else
					If Not StringRegExp($sTitle,$sTitleFilter) Then $bMatchMade=False
			EndSwitch
		EndIf
		If $bMatchMade Then
			$iTotal+=1
		...
		$bMatchMade=True
		; Next module
		$aRet=DllCall($_PFhKern32DLL,"bool","Module32NextW","handle",$hTlHlp,"ptr",$pMEPointer)
	WEnd
	__PFCloseHandle($hTlHlp)
...
;		@error = -1 = more than one module with the *same name* were found (unlikely). The *first* module's address is returned
;	Failure: -1 with @error set:
;		@error = 1 = invalid parameter, or process does not exist.
;		@error = 2 = DLLCall() error. @extended = actual DLLCall error code
;		@error = 3 = INVALID_HANDLE_VALUE returned from __PFCreateToolHelp32Snapshot()
...
Func _ProcessGetModuleBaseAddress($vProcessID,$sModuleName,$bList32bitMods=False)
	Local $aModList=_ProcessListModules($vProcessID,$sModuleName,0,$bList32bitMods)
	If @error Then Return SetError(@error,@extended,-1)
	If $aModList[0][0]=0 Then Return SetError(-1,0,-1)
	; *unlikely* scenario (more than one module naame matches) - set error, but return 1st match
	If $aModList[0][0]>1 Then SetError(-1)
...
Func _ProcessListWTS($vFilter=0,$iMatchMode=0,$vWTSAPI32DLL="wtsapi32.dll")
	...
	Local $iOffset=0,$aSidInfo,$bMatchMade=True,$iTotal=0,$bFilterOn=False
	...
	For $i=1 To $iTotalStructs
		...
		$aRet=DllCall($_PFhKern32DLL,"ptr","lstrcpynW","ptr",$pStBuf,"ptr",DllStructGetData($stWTSProcInfo,3),"int",260)
...
Func _ProcessWinList($vProcessID,$sTitle=0,$bOnlyGetVisible=False,$bOnlyGetRoot=False,$bOnlyGetAltTab=False)
	If Not IsNumber($vProcessID) Then
		...
		If $vProcessID=0 Then Return SetError(1,0,"")
	EndIf
	If $bOnlyGetAltTab Then $bOnlyGetVisible=True	; Alt-Tab windows must be visible too, so ensure this param is True.
	Local $aWinList,$aEnumList,$hWndCur,$iCount=0,$iCurMatchCriteria=0,$iTotalCriteriaToMatch=0
	...
	If $bOnlyGetAltTab Then $iTotalCriteriaToMatch+=1
	...
	For $i=1 To $aWinList[0][0]
		...
		If $vProcessID=WinGetProcess($hWndCur) Then
			...
			; $GWL_EXSTYLE	= 0xFFFFFFEC <Constants.au3> $WS_EX_TOOLWINDOW = 0x80
			If $bOnlyGetAltTab And BitAND(_WinAPI_GetWindowLong($hWndCur,0xFFFFFFEC),0x80)=0 Then $iCurMatchCriteria+=1
