#include <Array.au3>
;#include <_ProcessFunctions.au3>	; already part of the below two
#include <_ProcessListFunctions.au3>
;~ #include <_ProcessUndocumented.au3>	; part of the below
#include <_ProcessUndocumentedList.au3>
#include <_WinTimeFunctions.au3>	; for _ProcessGetTimes() conversions
;	- KODA GUI INCLUDES	-
...
; ===============================================================================================================================
; <TestProcessFunctions.au3>
;
;	Test of _ProcessFunctions, _ProcessListFunctions, _ProcessUndocumented, & _ProcessUndocumentedList modules
;
; Function:
;	TestProcessGUI()
;
; Internal functions:
;	_GetPrivilege_SeDebugRtl()	; Sets SEDEBUG privilege
...
Global $aAccessSecCtls,$aPFGProcList[1][2]=[ ["Self",$PROCESS_THIS_HANDLE] ]
...
Dim $aProcessFuncs[43][6]=[ ["_ProcessGetPID",0,0x1000,"_ProcessUDGetPID",$sY,$sH], ["_ProcessGetExitCode",0,0x1000,0,$sY,$sH], _
	...
	["_ProcessGetGUIResources",0,0x1000,0,$sY,$sH&',$iObjType,$vUSER32DLL="user32.dll"'], _
	...
	["_ProcessMemoryFree",0x8,0,0,$sN,$sH&",$pAddress,$iNumBytes=0,$iFreeType=0x8000"], _
	...
	["_ProcessUDGetHeaps",0x10,0x1000,0,$sN,$sH], _
	...
	["_ProcessGetModuleBaseAddress",0,0,0,$sN,$sP&",$sModuleName,$bList32bitMods=False"], _
	...
	["_ProcessUDListEverything",0,0,0,$sY,$sF&",$bEnumThreads=True"] ]
...
; ====================================================================================================
; Func _GetPrivilege_SeDebugRtl()
;
; Performs the same function as _GetPrivilege_SEDEBUG() [Obtains SE_DEBUG privilege for the running process]
;	The function (without my changes) was originally posted by Manko as '_GetPrivilege_SeDebug()' in
;	 the thread "Get SeDebug privilege" @ http://www.autoitscript.com/forum/index.php?showtopic=111061
;
; Reference: See "Tip:Easy way to enable privileges" - Sysinternals Forum @
...
Func _GetPrivilege_SeDebugRtl()
    Local $aRet=DllCall("ntdll.dll","long","RtlAdjustPrivilege","ulong",20,"bool",True,"bool",False,"bool*",0) ; 20 is SeDebug privilege...
    If @error Then Return SetError(2,@error,False)
    If $aRet[0] Then SetError(3,$aRet[0],False)
	Return True
EndFunc
...
Func __UpdateProcessList()
	...
	If $iCurBound-1<>$aProcList[0][0]-2 Or $aProcList[$aProcList[0][0]][1]<>$aPFGProcList[$iCurBound-1][1] Then
		...
		; -2 for System and Idle processes, +1 for 'Self'
		Dim $aPFGProcList[$aProcList[0][0]-2+1][2]
		$aPFGProcList[0][0]="Self"
		...
		For $i=1 To $aProcList[0][0]
			...
			If $aProcList[$i][1]=0 Or $aProcList[$i][0]="System" Then ContinueLoop
			...
			$iListIndex+=1
		Next
		If $sPrevSel="" Or Not StringInStr($sList,$sPrevSel) Then $sPrevSel="Self"
...
Func __UpdateAccessByControl($ctControl,$iBitValue,$ctLinkedControl=-1)
	Local $bChecked=False,$aSelectedItems
	...
	For $i=1 To $aSelectedItems[0]
		For $i2=0 To UBound($aProcessFuncs)-1
			; Matching text
			If _GUICtrlListBox_GetText($hPFGFunctionsList,$aSelectedItems[$i])=$aProcessFuncs[$i2][0] Then
...
Func __ReRunGUI($hParentGUI)
	If IsAdmin() And (@OSArch="X86" Or @OSArch="IA64" Or @Compiled) Then _
	...
	Local $sAdminCB,$sBitModeCB,$bBitModeChangeAvailable=False,$sExeName=@AutoItExe,$sParam="",$sVerb="",$bRestartReady=False
	...
	If GUICtrlRead($RSSBitModeCB)=$GUI_CHECKED And Not @Compiled Then
		If @AutoItX64 Then
...
Func TestProcessGUI()
	...
	#Region ### START Koda GUI section ###
	$PFGUI = GUICreate("Process Functions Tester", 599, 535, 192, 124)
...
;~ 				ConsoleWrite("DropDown clicked, selection="&GUICtrlRead($PFGProcDropDown)&@CRLF)
				$sTemp=GUICtrlRead($PFGProcDropDown)
				For $i=0 To UBound($aPFGProcList)-1
					If $sTemp=$aPFGProcList[$i][0] Then
						If $aPFGProcList[$i][1]<>$PROCESS_THIS_HANDLE And Not ProcessExists($aPFGProcList[$i][1]) Then __UpdateProcessList()
						ExitLoop
					EndIf
				Next
...
;~ 				ConsoleWrite("Folder path:"& $sLastRunDir&", File to Run:"&$sLastRunExe&@CRLF)
			...
			Case $PFGDebugPrivBtn
				...
				If _GetPrivilege_SeDebugRtl() Then
...
Func __ExecuteExamples($sDropDownSel)
	Local $hOutputGUI,$iErr,$iTemp,$aTemp,$pTemp,$bTemp,$sTemp,$pDest,$aEnv,$hProcessTemp
	...
	For $i=0 To UBound($aPFGProcList)-1
		If $sDropDownSel=$aPFGProcList[$i][0] Then
			$iProcessID=$aPFGProcList[$i][1]
			If $iProcessID<>$PROCESS_THIS_HANDLE And Not ProcessExists($iProcessID) Then
				Return MsgBox(48,"Process does not exist","Process selected from DropDown ("&$sDropDownSel&") does not exist")
			EndIf
			ExitLoop
		EndIf
	Next
	If $iProcessID=0 Then Return False	; this shouldn't happen
	If $iProcessID=$PROCESS_THIS_HANDLE Then $iProcessID=@AutoItPID

	#Region ### START Koda GUI section ###
	$hOutputGUI = GUICreate("Process Functions Test", 578, 403, 196, 124, BitOR($WS_MINIMIZEBOX,$WS_SIZEBOX,$WS_THICKFRAME,$WS_SYSMENU,$WS_CAPTION,$WS_POPUP,$WS_POPUPWINDOW,$WS_GROUP,$WS_BORDER,$WS_CLIPSIBLINGS))
	...
	#EndRegion ### END Koda GUI section ###
#cs
	; Create the output GUI
	$hOutputGUI = GUICreate("Process Functions Test",700,400,-1,-1,BitOR($WS_MINIMIZEBOX,$WS_CAPTION,$WS_POPUP,$WS_SYSMENU,$WS_THICKFRAME))
	$ctOutputEdit = GUICtrlCreateEdit("",2,2,696,396,0x200000)	; $WS_VSCROLL=0x00200000
	GUICtrlSetLimit($ctOutputEdit,1000000)
	GUICtrlSetFont($ctOutputEdit, 9, 400, 0, "Courier New")
	; Disable close button/menu item
	_GUICtrlMenu_EnableMenuItem(_GUICtrlMenu_GetSystemMenu($hOutputGUI),$SC_CLOSE,1,False)

	GUISetState(@SW_SHOW)
#ce
	If $sDropDownSel="Self" Then
	...
	_OutputWrite("Process '"&$sProcName&"' selected, Access: 0x"&Hex($iPFGAccessTotal)&", DebugPrivileges="&$bDebugPrivilegeObtained&@CRLF& _
		"$iProcessID="&$iProcessID&@CRLF&$sSeparator)
	_OutputWrite("_Process* Functions selected for testing:")
	For $i=1 To $aSelectedItems[0]
		_OutputWrite("#"&$i&":"&$aSelectedItems[$i])
	Next
	_OutputWrite($sSeparator)
	If $iPFGAccessTotal Then
		_OutputWrite("_ProcessOpen($vProcessID,$iAccess,$bInheritHandle=False)"&@CRLF)
	...
	For $i=1 To $aSelectedItems[0]
		...
		Switch $sSelItem
			...
			Case "_ProcessSetAffinityMask"
				...
				Else
					_OutputWrite("_ProcessSetAffinityMask() not called, due to inability to retrieve current Priority with _ProcessGetAffinityMask()")
				EndIf
			...
			Case "_ProcessGetGUIResources"
				; Func _ProcessGetGUIResources($hProcess,$iObjType,$vUSER32DLL="user32.dll")
			...
			Case "_ProcessMemoryWrite"
				; Func _ProcessMemoryWrite($hProcess,$pDest,$pSource,$iNumBytes)
				_OutputWrite("$bMemoryWritten=_ProcessMemoryWrite($hProcess,$pDest,$pSource,$iNumBytes)")
				_OutputWrite("---- Not called directly. (Suggestions for 'safe' examples welcome...) ----")
			Case "_ProcessMemoryAlloc","_ProcessMemoryFree"
				; Func _ProcessMemoryAlloc($hProcess,$iNumBytes,$iAllocType,$iProtectType,$pAddress=0)
				; Func _ProcessMemoryFree($hProcess,$pAddress,$iNumBytes=0,$iFreeType=0x8000)
			...
			Case "_ProcessMemoryVirtualQuery"
				...
				Endif
			Case "_ProcessMemoryVirtualProtect"
				; Func _ProcessMemoryVirtualProtect($hProcess,$pAddress,$iNumBytes,$iProtect)
				_OutputWrite("$bProtectAltered=_ProcessMemoryVirtualProtect($hProcess,$pAddress,4096,$PAGE_READWRITE)")
				_OutputWrite("---- Not called directly. (Suggestions for 'safe' examples welcome...) ----")
			...
			Case "_ProcessGetChildren"
				...
				If Not $iErr Then
					...
					If $aTemp[0][0]=0 Then
						...
						If $sDropDownSel="Self" Then _OutputWrite(" - use the 'Run New Program' command from the main GUI",False)
						_OutputWrite("")
					Else
						$aTemp[0][0]="Process Name"
						$aTemp[0][1]="Process ID#"
...
#ce
			...
			Case "_ProcessListModules"
				...
				_OutputWrite("$aModules=_ProcessListModules($iProcessID)",False)
				$aTemp=_ProcessListModules($iProcessID)
				$iErr=@error
				_OutputWrite("    ; @error="&@error&" @extended="&@extended)
				If Not $iErr Then
...
#ce
			...
			Case "_ProcessGetModuleBaseAddress"
				; Func _ProcessGetModuleBaseAddress($vProcessID,$sModuleName,$bList32bitMods=False)
			...
			Case "_ProcessUDListEverything"
				...
				If Not $iErr Then
					...
					Else
						...
						$aTemp[0][0]="Thread ID"
						$aTemp[0][1]="Process ID"
						$aTemp[0][2]="Thread Address (not Base Address)"
