...
;		- Read/Write Memory Functions would fail also, and _ProcessGetMemInfo() may report incorrect info (or nothing at all)
;
; General Functions:
...
;
; +----------------------------------------------------------------------------------------------------------------------+
; <_ProcessListFunctions.au3> (external UDF) Functions 'Preview':
; +----------------------------------------------------------------------------------------------------------------------+
; List/Enumeration Functions (utilizing CreateToolHelp32Snapshot - except _ProcessWinList()):
...
;
; To consider:
;	_ProcessGetCPUUsage()	; This can't be accomplished in one call, I believe, so is better left as an external UDF
;							;	(PDH Performance Counters may be suitable here)
;
; INTERNAL Functions:
;	__PFCloseHandle()		; basically same as _WinAPI_CloseHandle()
;
...
Global $_PFhKern32DLL=DllOpen("kernel32.dll")	; DLLClose() will be done automatically on exit. [this doesn't reload the DLL]
...
Func __PFCloseHandle(ByRef $hHandle)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","CloseHandle","handle",$hHandle)
...
Func __PFBuildDeviceToDriveXlation()
	...
	For $i=1 To $aDriveArray[0]
		...
		$aRet=DllCall($_PFhKern32DLL,"dword","QueryDosDeviceW","wstr",$_PFaDeviceToDriveMap[$i-1][0],"wstr",0,"dword",65536)
		If @error Then
...
Func _ProcessOpen($vProcessID,$iAccess,$bInheritHandle=False)
	...
	If $vProcessID=-1 Then
		$aRet=DllCall($_PFhKern32DLL,"handle","GetCurrentProcess")
		If @error Then Return SetError(2,@error,0)
		Return $aRet[0]		; usually the constant '-1', but we're keeping it future-OS compatible this way
	ElseIf Not IsNumber($vProcessID) Then
		; This allows process name OR Process ID (but ultimately converts to PID)
		$vProcessID=ProcessExists($vProcessID)
		; Process doesn't exist? (or possibly invalid param)
		If $vProcessID=0 Then Return SetError(16,0,0)
	EndIf
	$aRet=DllCall($_PFhKern32DLL,"handle","OpenProcess","dword",$iAccess,"bool",$bInheritHandle,"dword",$vProcessID)
	If @error Then Return SetError(2,@error,0)
...
Func _ProcessGetPID($hProcess)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"dword","GetProcessId","handle",$hProcess)
	If @error Then Return SetError(2,@error,0)
...
Func _ProcessGetExitCode($hProcess)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","GetExitCodeProcess","handle",$hProcess,"dword*",0)
...
Func _ProcessIsWow64($hProcess)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","IsWow64Process","handle",$hProcess,"bool*",0)
	If @error Then
...
Func _ProcessGetPriorityX($hProcess)
	If Not IsPtr($hProcess) Then Return SetError(1,0,-1)
	Local $aRet=DllCall($_PFhKern32DLL,"int","GetPriorityClass","handle",$hProcess)
	If @error Then Return SetError(2,@error,-1)
...
Func _ProcessSetPriorityX($hProcess,$iPriority=0x20)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","SetPriorityClass","handle",$hProcess,"int",$iPriority)
	If @error Then Return SetError(2,@error,False)
...
Func _ProcessGetAffinityMask($hProcess)
	If Not IsPtr($hProcess) Then Return SetError(1,0,0)
	Local $aRet=DllCall($_PFhKern32DLL,"bool","GetProcessAffinityMask","handle",$hProcess,"dword_ptr*",0,"dword_ptr*",0)
	If @error Then Return SetError(2,@error,0)
...
Func _ProcessSetAffinityMask($hProcess,$iAffinityMask)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","SetProcessAffinityMask","handle",$hProcess,"dword_ptr",$iAffinityMask)
	If @error Then Return SetError(2,@error,False)
	If Not $aRet[0] Then Return SetError(3,0,False)	; failed
	Return True
EndFunc
...
Func _ProcessGetTimes($hProcess,$iTimeToGet=-1)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","GetProcessTimes","handle",$hProcess,"uint64*",0,"uint64*",0,"uint64*",0,"uint64*",0)
	If @error Then Return SetError(2,@error,-1)
	If Not $aRet[0] Then Return SetError(3,0,-1)
	; Return ALL?
...
Func _ProcessGetOwner($hProcess,$vADVAPI32DLL="advapi32.dll")
	...
	$aRet=DllCall($_PFhKern32DLL,"handle","LocalFree","handle",$aRet[8])
	If @error Then
		SetError(-2,@error)
	ElseIf $aRet[0] Then
		SetError(-3,$aRet[0])
...
Func _ProcessGetHandleCount($hProcess)
	If Not IsPtr($hProcess) Then Return SetError(1,0,-1)
	Local $aRet=DllCall($_PFhKern32DLL,"bool","GetProcessHandleCount","handle",$hProcess,"dword*",0)
	If @error Then Return SetError(2,@error,-1)
...
Func _ProcessGetIOCounters($hProcess,$iCounterToGet=-1)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","GetProcessIoCounters","handle",$hProcess,"ptr",DllStructGetPtr($stIOCounters))
	If @error Then Return SetError(2,@error,-1)
	If Not $aRet[0] Then Return SetError(3,@error,-1)
	; Return ALL?
...
Func _ProcessGetGUIResources($hProcess,$iObjType,$vUSER32DLL="user32.dll")
	If Not IsPtr($hProcess) Or $iObjType<0 Or $iObjType>4 Or $vUSER32DLL<0 Then Return SetError(1,0,-1)
	Local $aRet=DllCall($vUSER32DLL,"dword","GetGuiResources","handle",$hProcess,"dword",$iObjType)
	If @error Then Return SetError(2,@error,-1)
	Return $aRet[0]
EndFunc
...
Func _ProcessGetPathname($hProcess,$bResetDriveMap=False,$vPSAPIDLL=-1)
	...
	If @OSVersion<>"WIN_XP" And @OSVersion<>"WIN_XPe" And @OSVersion<>"WIN_2003" Then	; If StringRegExp(@OSVersion,"_(XP|2003)")=0
		...
		Else
			$aRet=DllCall($_PFhKern32DLL,"bool","QueryFullProcessImageNameW","handle",$hProcess,"dword",0,"wstr","","dword*",65536)
		EndIf
		If @error Then Return SetError(2,@error,"")
		If Not $aRet[0] Then Return SetError(3,0,"")	; fail
		Return $aRet[3]
...
Func _ProcessMemoryRead($hProcess,$pSource,$pDest,$iNumBytes)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","ReadProcessMemory","handle",$hProcess,"ptr",$pSource,"ptr",$pDest,"ulong_ptr",$iNumBytes,"ulong_ptr*",0)
	If @error Then Return SetError(2,@error,False)
...
Func _ProcessMemoryWrite($hProcess,$pDest,$pSource,$iNumBytes)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","WriteProcessMemory","handle",$hProcess,"ptr", $pDest,"ptr",$pSource,"ulong_ptr",$iNumBytes,"ulong_ptr*",0)
	If @error Then Return SetError(2,@error,False)
...
Func _ProcessMemoryAlloc($hProcess,$iNumBytes,$iAllocType,$iProtectType,$pAddress=0)
	If Not IsPtr($hProcess) Then Return SetError(1,0,0)
	Local $aRet=DllCall($_PFhKern32DLL,"ptr","VirtualAllocEx","handle",$hProcess,"ptr",0,"ulong_ptr",$iNumBytes,"dword",$iAllocType,"dword",$iProtectType)
	If @error Then Return SetError(2,@error,0)
...
; $iNumBytes = total # of bytes to allocate. Use 0 if FreeType = MEM_RELEASE (0x8000)
...
Func _ProcessMemoryFree($hProcess,$pAddress,$iNumBytes=0,$iFreeType=0x8000)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","VirtualFreeEx","handle",$hProcess,"ptr",$pAddress,"ulong_ptr",$iNumBytes,"dword",$iFreeType)
	If @error Then Return SetError(2,@error,False)
	If Not $aRet[0] Then Return SetError(3,0,False)
	Return True
EndFunc
...
Func _ProcessMemoryVirtualQuery($hProcess,$pAddress,$iInfo=-1)
	...
	$aRet=DllCall($_PFhKern32DLL,"ulong_ptr","VirtualQueryEx","handle",$hProcess,"ptr",$pAddress,"ptr",DllStructGetPtr($stMemInfo),"ulong_ptr",$iStrSz)
	If @error Then Return SetError(2,@error,-1)
	If Not $aRet[0] Then Return SetError(3,@error,-1)
...
Func _ProcessMemoryVirtualProtect($hProcess,$pAddress,$iNumBytes,$iProtect)
	...
	Local $aRet=DllCall($_PFhKern32DLL,"bool","VirtualProtectEx","handle",$hProcess,"ptr",$pAddress,"ulong_ptr",$iNumBytes,"dword",$iProtect,"dword*",0)
	If @error Then Return SetError(2,@error,False)
	If Not $aRet[0] Then Return SetError(3,0,False)
